  ...
  return (
    <div className="space-y-3 bg-white dark:bg-stone-900 p-4 rounded-2xl border border-stone-200 dark:border-stone-800 shadow-2xs">
      ...
      <div className="flex flex-col md:flex-row items-center gap-3">
        ...
        {/* Rep selector + stale toggle */}
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 w-full md:w-auto">
          <select
            aria-label="المسؤول عن العميل"
            value={selectedRep}
            onChange={(e) => onRepChange(e.target.value)}
            className="flex-1 sm:flex-none rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800 px-3 py-2 text-xs font-medium text-slate-700 dark:text-stone-200 focus:border-red-500 focus:outline-hidden"
          >
          ...
          <button
            ...
            className={cn(
              "flex-1 sm:flex-none flex items-center justify-center gap-1.5 rounded-xl px-3 py-2 text-xs font-semibold transition-colors border shrink-0",
              staleOnly
                ? "bg-amber-500 text-white border-amber-600 shadow-2xs"
                : "bg-white dark:bg-stone-800 text-slate-600 dark:text-stone-300 border-slate-200 dark:border-stone-700 hover:bg-slate-50 dark:hover:bg-stone-700"
            )}
          >
      ...
      {/* Bottom Row: Segment Pills without raw scrollbars */}
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pt-2 pb-1 border-t border-slate-100 dark:border-stone-800">
